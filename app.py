...
def transcribe_chunk(chunk, chunk_number):
    ...
    temp_files.append(chunk_name)  # Add temporary file to list
...
def check_and_convert_to_128kbps(file_path):
    ...
    if file_extension == ".mp3":
        ...
        if bitrate != 128000:  # 128 kbps is 128,000 bits per second
            ...
            temp_files.append(output_path)  # Add temporary file to list
...
openai.api_key = st.text_input("Enter OpenAI Api Key",
                               help="You need an account on OpenAI to generate a key: https://openai.com/blog/openai-api")
        ...
        with st.spinner("Crystallizing"):
            if voice_memo is not None:
                ...
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temporary_file:
                    ...
                    temp_files.append(temp_file_path)  # Add temporary file to list
